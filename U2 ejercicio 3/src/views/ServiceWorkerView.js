...
const SCOPE_TEST_PATHS = [
  ...
  { label: "Ruta del router", path: `${BASE_PATH}/mi-lista` },
  { label: "Ruta del router con parámetro", path: `${BASE_PATH}/destino/2` },
  { label: "Archivo interno de la app", path: `${BASE_PATH}/src/main.js` },
  { label: "Raíz SIN diagonal final", path: BASE_PATH },
  { label: "Otra carpeta del repo (backend)", path: `${PARENT_SCOPE}backend/` },
...
function renderStatus(status) {
  ...
  const rows = [
    ["¿El navegador soporta Service Workers?", yes("Sí")],
    ...
    ["URL del script", status.scriptURL ? `<code>${status.scriptURL}</code>` : "—"],
...
function renderScopeTable(scope) {
  // Si todavía no hay registro, comparamos contra el scope que pediremos
...
async function tryWideScope() {
  ...
  try {
    ...
    console.error("[PWA] Scope inválido rechazado:", error);
...
async function unregisterServiceWorker() {
  ...
  showResult(
    removed
      ? "SW dado de baja. Esta pestaña sigue controlada hasta que recargues (F5); al recargar se vuelve a registrar."
